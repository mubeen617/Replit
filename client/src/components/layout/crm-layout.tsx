...
export default function CRMLayout({ user, userType, onLogout, children }: CRMLayoutProps) {
  ...
  const getInitials = (user: Customer | CustomerUser) => {
    if (userType === "customer") {
      const customer = user as Customer;
      return customer.adminName ? customer.adminName.split(' ').map(n => n[0]).join('').toUpperCase() : "CM";
    } else {
      const customerUser = user as CustomerUser;
      return `${customerUser.firstName?.[0] || ''}${customerUser.lastName?.[0] || ''}`.toUpperCase() || "CU";
    }
  };

  const getUserDisplayName = (user: Customer | CustomerUser) => {
    if (userType === "customer") {
      const customer = user as Customer;
      return customer.adminName || "Customer Manager";
    } else {
      const customerUser = user as CustomerUser;
  ...
  const getUserRole = (user: Customer | CustomerUser) => {
    if (userType === "customer") {
      ...
      <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
        {navigation.map((item) => {
          const isActive = location === item.href || 
            (item.href === "/crm/dashboard" && location === "/crm");
          return (
            <Link key={item.name} href={item.href}>
              <div
                className={`flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-colors cursor-pointer group ${
                  isActive
